...
def parse_revision_entries(page_html: str) -> list[RevisionEntry]:
    ...
    for match in row_pattern.finditer(page_html):
        ...
        base_revision = re.fullmatch(r"Revision\s+([A-Z])", description)
...
def latest_base_revision(entries: list[RevisionEntry]) -> RevisionEntry:
    if not entries:
...
def resolve_latest_revision_download(
    ...
    revision = latest_base_revision(parse_revision_entries(details_html))
...
def download_latest_revision_bytes(
    ...
    session = session or AssistSession()
    resolved = resolve_latest_revision_download(ident_number, document_key, session=session)
